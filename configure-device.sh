...
# Makima Device Configuration Helper
...
CONFIG_DIR="$USER_HOME/.config/makima"
...
list_devices() {
    ...
    if command -v evtest &> /dev/null; then
        ...
        sudo evtest 2>/dev/null | grep -E "^/dev/input/event[0-9]+" | while read -r line; do
...
get_device_name() {
    ...
    # Try to get device name using evtest
    if command -v evtest &> /dev/null; then
        timeout 2s evtest "$event_device" 2>/dev/null | head -1 | sed 's/.*device: //' | sed 's/Input driver version.*//' | xargs
    else
        # Fallback method
...
# Generated by configure-device.sh
#
# This configuration uses Ruby scripting for advanced input processing.
# Set the MAKIMA_RUBY_SCRIPT environment variable to your Ruby script path.
...
# Uncomment and set the path to your Ruby script:
# RUBY_SCRIPT = "$USER_HOME/.config/makima/scripts/my_script.rb"

# Example usage:
# export MAKIMA_RUBY_SCRIPT="$USER_HOME/.config/makima/examples/ruby_scripts/eat_input.rb"
# systemctl --user restart makima
EOF

    # Create example Ruby script if it doesn't exist
    local script_dir="$CONFIG_DIR/scripts"
    mkdir -p "$script_dir"

    if [ ! -f "$script_dir/example.rb" ]; then
        cat > "$script_dir/example.rb" << 'EOF'
# Example Ruby script for Makima
# This script demonstrates the basic structure and available functions
...
def handle(event)
  ...
  # Available functions:
  # - Makima.press(key_code): Press and release a key
  # - Makima.press_down(key1, key2, ...): Press keys down (don't release)
  # - Makima.release(key_code): Release a key

  # Key constants:
  # - Makima::KB_LALT, Makima::KB_LTAB, Makima::KB_ESC, Makima::KB_ENTER

  # Return values:
  # - return nil: consume the event (don't pass it through)
  # - return anything else or no return: pass the event through

  # Example 1: Convert Caps Lock to Escape
  if event.key == 58 && event.key_down?  # KEY_CAPSLOCK = 58
    Makima.press(1)  # KEY_ESC = 1
    return nil  # Consume the original event
  end

  # Example 2: Alt+Tab combo on any key
  # if event.key_down?
  #   Makima.press_down(Makima::KB_LALT, Makima::KB_LTAB)
  #   return nil
  # end

  # Default: pass through the event unchanged
end
EOF
        print_success "Example Ruby script created: $script_dir/example.rb"
    fi
...
interactive_setup() {
    print_status "=== Makima Device Configuration ==="
    echo
    ...
    echo "2. Test your configuration: sudo systemctl restart makima"
    echo "3. Check logs: journalctl -u makima -f"
}

show_help() {
    echo "Makima Device Configuration Helper"
    echo
