      ...
      match server_str {
        "Hyprland" => {
          ...
          if let Ok(reply) = serde_json::from_str::<serde_json::Value>(std::str::from_utf8(query.stdout.as_slice()).unwrap()) {
            ...
            if let Some(_) = config.iter().find(|&x| x.associations.client == active_window) { // TODO: repeated verbatim
              active_window
            } else {
              Client::Default
            }
        ...
        "niri" => {
          ...
          if let Ok(reply) = serde_json::from_str::<serde_json::Value>(std::str::from_utf8(query.stdout.as_slice()).unwrap()) {
            let active_window = Client::Class(reply["app_id"].to_string().replace("\"", ""));
